...
returnStopEvent(result=f"Hello, {name}!")
...
curlhttp://localhost:80/events/someUniqueId123?sse=falseacquire_timeout=1include_internal=falseinclude_qualified_name=true
...
[Section titled “Using WorkflowClient to Interact with Servers”](https://developers.llamaindex.ai/python/llamaagents/workflows/deployment/#using-workflowclient-to-interact-with-servers)
...
asyncfor event in client.get_workflow_events(handler_id=handler_id):
...
returnRequestEvent(prompt="What is your name?")
...
returnOutEvent(output=f"Hello, {ev.response}")
