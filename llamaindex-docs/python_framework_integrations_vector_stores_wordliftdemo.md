...
ifisinstance(row.get("product_description"),)
...
ifisinstance(row.get("product_price"),)
...
ifisinstance(row.get("product_category"),)
...
awaitcreate_entity(entities_api, entity_data)
...
asyncwithApiClient(configuration) as api_client:
...
awaitcleanup_knowledge_graph(api_client)
...
f"Failed to clean up the existing Knowledge Graph: (e)}"
...
awaitbuild_knowledge_graph(pages_df, CRAWL_URL, api_client)
...
awaitperform_graphql_query(api_client)
...
similar_urls =awaitget_top_k_similar_urls(
...
vector_store =WordliftVectorStore=API_KEY)
