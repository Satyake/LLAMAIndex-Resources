# Sbert rerank
##  SentenceTransformerRerank [#](https://developers.llamaindex.ai/python/framework-api-reference/postprocessor/sbert_rerank/#llama_index.postprocessor.sbert_rerank.SentenceTransformerRerank "Permanent link")
Bases: 
HuggingFace class for cross encoding two sentences/texts.
Parameters:
Name | Type | Description | Default  
---|---|---|---  
`model` |  A model name from Hugging Face Hub that can be loaded with AutoModel, or a path to a local model. |  `'cross-encoder/stsb-distilroberta-base'`  
`device` |  Device (like “cuda”, “cpu”, “mps”, “npu”) that should be used for computation. If None, checks if a GPU can be used. |  `None`  
`cache_folder` |  `(str, Path)` |  Path to the folder where cached files are stored. Defaults to None. |  `None`  
`top_n` |  Number of nodes to return sorted by score. Defaults to 2.  
`keep_retrieval_score` |  `bool` |  Whether to keep the retrieval score in metadata. Defaults to False. |  `False`  
`cross_encoder_kwargs` |  `dict` |  Additional keyword arguments for CrossEncoder initialization. Defaults to None. |  `None`  
...
---|---  
options: members: - SentenceTransformerRerank
