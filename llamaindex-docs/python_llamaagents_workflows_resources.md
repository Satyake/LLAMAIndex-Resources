...
returnSecondEvent="This is an input for step 2")
...
returnStopEvent(result="Messages put into memory")
...
returnStopEvent(result=f"Using threshold: {config.threshold}")
...
returnStopEvent(result=response.text)
