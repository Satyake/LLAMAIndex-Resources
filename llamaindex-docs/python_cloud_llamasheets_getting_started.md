...
assert result.regions isnotNone
...
assert region.region_id isnotNone
...
withopen(f"./downloaded_region_{region.region_id}.parquet","wb") as f:
...
withopen(f"./downloaded_region_{region.region_id}_metadata.parquet","wb") as f:
...
for (const regionof result.regions) {
...
print(f"Found (result.regions)} region(s)")
...
for (const worksheetMetadataof result.worksheet_metadata|| []) {
